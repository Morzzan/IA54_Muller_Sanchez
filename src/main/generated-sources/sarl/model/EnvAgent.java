...
public class EnvAgent extends Agent {
  ...
  private void $behaviorUnit$StartEvent$1(final StartEvent occurrence) {
    ...
    for (int i = 0; (i < this.nbuavs); i++) {
      ...
      _$CAPACITY_USE$IO_SARL_CORE_LIFECYCLE$CALLER.spawn(UAVAgent.class, Integer.valueOf(i));
    }
  }
  ...
  @Pure
  protected ArrayList<Vector3D> getNeighbors(final UAVBody uav) {
  ...
  protected void runstep() {
    ...
    for (final GoThatWay action : this.actions) {
      {
        UAVBody actorBody = this.uavs.get(action.getSource().getUUID());
    ...
    this.fx.moveUAVs(this.uavs.values());
    this.actions.clear();
    this.sendPercepts();
  }
  ...
  public int hashCode() {
    ...
    final int prime = 31;
    result = prime * result + this.nbsteps;
